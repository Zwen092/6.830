...
//        // some code goes here
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        /*
        step1: generate a new page to store half entries
        step2: move half of the tuples to the new page
        step3: find the parent page and copy up the leftmost entry to this parent and call updateEntry() / insertEntry()
        suppose the parent entry has empty slot
        step3.5: update the sibling pointers
        step4: make the right page point to the parent page
        step5: done?
         */
        /*
...
//        }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        newPage.deleteKeyAndLeftChild(temp);
        // maybe i should update the entry in the parent page
        Field insertField = temp.getKey();
        BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), insertField);
        Iterator<BTreeEntry> iterator = parentPage.iterator();
        BTreeEntry entryToUpdate = null;
        while (iterator.hasNext()) {
            entryToUpdate = iterator.next();
            if (insertField.compare(Op.LESS_THAN_OR_EQ, entryToUpdate.getKey()))
                break;
        }
        if (iterator.hasNext()) {
            entryToUpdate.setLeftChild(newPage.getId());
            parentPage.updateEntry(entryToUpdate);
        }
        BTreeEntry entryToInsert = new BTreeEntry(temp.getKey(), page.getId(), newPage.getId());
        ...
        //consider which page to return
        if (field.compare(Op.GREATER_THAN_OR_EQ, temp.getKey())) {
            return newPage;
        } else {
            return page;
        }
...
//        dirtypages.put(parent.getId(), parent);
//
//        if (field.compare(Op.GREATER_THAN_OR_EQ, midKey.getKey())) {
